...
static void phase1(Aircraft *aircrafts,int count,int X,double ref_lat,double ref_lon,const char *csv_name){
    ...
    printf("[Time] Phase 1 (Vincenty) compute runtime: %.3f ms\n",elapsed_time);
...
static void phase2(Aircraft *aircrafts,int count,int X,double ref_lat,double ref_lon, const char *csv_name){
    ...
    double elapsed_time = elapsed_ms(t0);
    int cores = omp_get_max_threads();
    printf("[Time] Phase 2 (Full Haversine) compute runtime: %.3f ms\n",elapsed_time);
...
static void phase3(Aircraft *aircrafts,int count,int X,double ref_lat,double ref_lon, const char *csv_name){
    ...
    double elapsed_time = elapsed_ms(t0);
    int cores = omp_get_max_threads();
    printf("[Time] Phase 3 (Half Haversine 'a') compute runtime: %.3f ms\n",elapsed_time);
...
int main(int argc,char *argv[]){
    ...
    if (args.phase < 1 || args.phase > 4){
        ...
        fprintf(stderr,
            "Example: %s --phase=3 30.4733 -87.1866 25 --data-file input/myfile.csv --use-rows 10000000\n", argv[0]);
        return 1;
    }
    ...
    /* Example: output/phase1-output-omp-8t.csv */
    static char auto_name[256];
    snprintf(auto_name, sizeof(auto_name),"output/%s-output-omp-%dt.csv", phase_tag, threads);
    ...
    if      (args.phase==1) phase1(aircrafts,count,args.topX,args.ref_lat,args.ref_lon,auto_name);
    else if (args.phase==2) phase2(aircrafts,count,args.topX,args.ref_lat,args.ref_lon,auto_name);
    else if (args.phase==3) phase3(aircrafts,count,args.topX,args.ref_lat,args.ref_lon,auto_name);
    else                    phase4(aircrafts,count,args.topX,args.ref_lat,args.ref_lon,auto_name);
